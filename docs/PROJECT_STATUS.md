...
**Production is not published.** ODR-024 stays closed: no Netlify live deploy, no DNS/domain, no analytics destination, and no indexation flip. The site remains `noindex, nofollow`. `/book` uses the Pavells embed (D-016) with WhatsApp, phone and walk-in fallbacks. First-party payments and notifications stay off. Do not treat the release-candidate tag as a go-live.
...
2. Owner decisions in `docs/OWNER_DECISIONS_REQUIRED.md` still block first-party payments, automated notifications, production host/domain, and consent-dependent media.
